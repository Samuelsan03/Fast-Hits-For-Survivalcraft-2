...
namespace Minecraft1_9HitUpdate
{
	...
	public class ComponentMcHit : Component, IUpdateable
	{
		...
		public void Hit(ComponentBody componentBody, Vector3 hitPoint, Vector3 hitDirection)
		{
			...
			Random random = this.ComponentMiner.m_random;
			double num10 = 0.275;
			...
			if (!this.ComponentMiner.CanUseTool(this.ComponentMiner.ActiveBlockValue))
			{
			...
			{
				bool result;
				modLoader.OnMinerHit(this.ComponentMiner, componentBody, hitPoint, hitDirection, ref num, ref num2, ref num3, ref result);
				return result;
			});
			num *= num11;
			...
			if (flag)
			{
				...
				float num5 = MathUtils.Pow(this.ComponentPlayer.ComponentBody.Mass / componentBody.Mass, 0.5f);
		...
		public void Update(float dt)
		{
			...
			{
				BodyRaycastResult? bodyRaycastResult = this.ComponentMiner.Raycast<BodyRaycastResult>(playerInput.Hit.Value, 1, true, true, true);
